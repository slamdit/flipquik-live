...
export default function Sales() {
  ...
  const handleEbaySync = async () => {
    setSyncing(true);
    try {
      const { data, error } = await supabase.functions.invoke('syncEbaySales', { body: {} });
  ...
  const { data: allItems = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return items.getAll({ filters: { user_id: user.id, status: 'sold' }, orderBy: '-updated_at', limit: 200 });
    }
  });

  const itemMap = Object.fromEntries(allItems.map(i => [i.id, i]));

  const totalRevenue = allSales.reduce((s, x) => s + (x.sold_price || 0), 0);
  const totalFees = allSales.reduce((s, x) => s + (x.fees || 0), 0);
  const totalProfit = allSales.reduce((s, x) => s + (x.net_profit || 0), 0);
  const avgROI = allSales.filter(x => x.roi_percent != null).length > 0
    ? allSales.filter(x => x.roi_percent != null).reduce((s, x) => s + x.roi_percent, 0) / allSales.filter(x => x.roi_percent != null).length
    : null;

  const platformCounts = allSales.reduce((acc, x) => {
    ...
    <div className="min-h-screen bg-slate-50 pb-24">
      <div className="bg-slate-900 text-white px-4 pt-4 pb-5">
        <div className="flex items-center gap-3">
          ...
          <span className="ml-auto text-slate-400 text-sm">{allSales.length} sold</span>
        </div>
        <div className="mt-3">
          <Button
                  ...
                  <div key={sale.id} className="bg-white rounded-xl shadow-sm flex items-center gap-3 p-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        {sale.platform && <span className="text-xs text-slate-400">{sale.platform}</span>}
                        {sale.sold_date && <span className="text-xs text-slate-400">{sale.sold_date}</span>}
                      </div>
                      <div className="flex items-center gap-3 mt-1">
                        ...
                        {sale.fees > 0 && <span className="text-xs text-slate-400">Fees: ${sale.fees?.toFixed(2)}</span>}
                      </div>
                    </div>
