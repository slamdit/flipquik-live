...
export default function EbayListButton({ item, onListed }) {
  ...
  const handleList = async () => {
    ...
    setListing(true);
    try {
      const { data, error } = await supabase.functions.invoke('publishToEbay', { body: { item_id: item.id, price: parseFloat(price) } });
      ...
      toast.error('eBay listing failed: ' + (e?.message || 'unknown error'));
    } finally {
      setListing(false);
    }
  };
  ...
  return (
    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 space-y-2">
      ...
      <div>
        ...
        <Input type="number" step="0.01" min="0" value={price} onChange={e => setPrice(e.target.value)} className="h-9 mt-1 text-sm" placeholder="0.00" />
      </div>
      <div className="flex gap-2">
        <Button size="sm" variant="outline" onClick={() => setShowForm(false)} className="flex-1 h-9 text-xs">Cancel</Button>
        <Button size="sm" onClick={handleList} disabled={listing} className="flex-1 h-9 text-xs bg-yellow-500 hover:bg-yellow-600 text-white">
